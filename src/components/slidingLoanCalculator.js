...
import { defaultLoanAmount, faq, interestRates, programLoanInfo, schoolName } from '../constants/programInfo';
...
const SlidingLoanCalculator = (props) => {
	...
	const [ nonPaymentPeriod, setNonPaymentPeriod ] = useState(programLoanInfo[0]['loanInfo']['Interest Only']['k']);
			...
			<div className="loanCalculator__content pt-8 bg-gray-100">
				<div className="loanCalculator__select flex flex-col items-center px-4">
					<div className="flex flex-col md:flex-row md:justify-between mb-8">
						<div className="flex flex-col md:w-1/2 justify-center px-4">
							...
							<p className="text-gray-800" id="leadText">
								Figuring out your monthly payments on a loan shouldn't require a math degree. Find out
								exactly what you'll pay at {schoolName} with a Skills Fund loan.
							</p>
						</div>
					...
					<div>
						...
						<label className={showLoanTypes ? 'text-sm' : 'hide'} htmlFor="program">
				...
				<div className="loanCalculator__monthlyPayments flex my-8">
					...
					>
						...
						<div className={loanType === 'Interest Only' ? 'show' : 'show move'}>
							<p className="loanCalculator__paymentAmounts text-3xl text-primary font-bold mb-1 text-center">
								${monthlyPayment.payment36}
							</p>
							<p className="loanCalculator__paymentLabel text-center text-xs">
								Monthly Payments{loanType === 'Interest Only' ? ' After Graduation' : null}
							</p>
							<p className="loanCalculator__paymentAmounts text-3xl text-primary font-bold mb-1 text-center">
								{formatterWithCents.format(totalPayment.payment36)}
							</p>
							<p className="loanCalculator__paymentLabel text-center text-xs">Total Cost of Loan</p>
						</div>
					</div>
                    ...
                    <span className="show"><Fragment><p className="loanCalculator__paymentAmounts text-3xl text-primary font-bold mb-1 text-center">$60,000</p><p className="loanCalculator__paymentLabel text-center text-xs">Annual Salary</p></Fragment></span>
                    <div>
				...
				<p className="text-center text-xs italic mb-0 pb-4 px-8">
					*The Annual Percentage Rate (APR) shown is estimated based on the loan type, origination fee, and
					approximate program length. The actual APR may be slightly different than the example provided based
					on loan type and program length. To learn how an Annual Percentage Rate (APR) is calculated, visit
					our{' '}
					<a className="text-secondary" href="https://skills.fund/resources/how-is-an-apr-calculated">
						blog
					</a>.
				</p>
			</div>
